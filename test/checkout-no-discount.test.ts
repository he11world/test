 ...
 *
 * `OrderRequest.discountCode` is declared optional and nullable, but
 * CheckoutService.createOrder dereferences it unconditionally. Any order
 ...
 *
 * The pre-existing suite supplies a discount code in every case, so this branch
 * was never exercised. These cases assert the correct behaviour: no discount
 * code means a zero discount and a null applied code (which is why `Order`
 * types `appliedCode` as `string | null`).
 */
